...
Entropy <- function( target, entropy_par = 1, type = "Shannon" ){
  ...
  if( type == "Shannon" | (entropy_par[1] == 1 & !type %in% c( "Sharma-Mittal", "Sharma-Taneja", "Kapur","Kaniadakis" ) ) ){
  ...
  } else if (type == "Kaniadakis" & entropy_par[1] != 0 ) {
    res = (target/sum(target))*( (target/sum(target))^entropy_par-(target/sum(target))^(-entropy_par) )/(2*entropy_par)
    res[target == 0] <- 0
    res = -sum(res)
  }
