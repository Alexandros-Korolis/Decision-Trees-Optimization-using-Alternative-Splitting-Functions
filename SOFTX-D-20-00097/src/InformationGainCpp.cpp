...
NumericVector EntropyCpp( NumericVector target, NumericVector entropy_par, String type ){
  ...
  }else if(type == "Kaniadakis"&& entropy_par[0] != 0) {
    double kappa = entropy_par[0];

    // Calculate Kaniadakis entropy: alpha != 0
    res = (target/sum(target))*( pow(target/sum(target),kappa)-pow(target/sum(target),-kappa) )/(2*kappa);
    res[target == 0] = 0;
    res = -sum(res);
}
